(() => {
    ...
    class StringAccumulatingSink {
        ...
        write(chunk) {
            this.accumulator.push(decoder.decode(chunk));
        }

        close() {
            this.consumer(this.accumulator.join(''));
        }

    }

    function accumStr(consumer) {
        return new WritableStream(new StringAccumulatingSink(consumer));
    }
    ...
    fetch('/static/html/includes.html')
        .then(request => request.body.pipeTo(accumStr(body => {
            const container = document.createElement('div');
            container.innerHTML = body;
            elemWrapper.insertBefore(container.getElementsByTagName('header')[0], elemMain);
            elemWrapper.insertBefore(container.getElementsByTagName('footer')[0], elemMain.nextSibling);
            ...
            })(0.001 + Math.random() * 2 * document.body.clientHeight / window.innerHeight), 10); // FIXME race condition
            ...
            document.fonts.ready.then(() => {
                resetNavHl();
                resizeStyleHandler();
                window.setTimeout(() => document.body.classList.add('loaded'), 10); // FIXME race condition
            });
        })));
